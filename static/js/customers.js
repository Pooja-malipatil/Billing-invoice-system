...
async function deleteCustomer(id) {
  ...
  try { await apiFetch(`/api/customers/${id}`, { method: "DELETE" }); showToast("Customer deleted"); loadCustomers(); }
  catch (err) { showToast(err.message, true); }
}
form.addEventListener("submit", async (e) => {
  ...
  try {
    if (id) { await apiFetch(`/api/customers/${id}`, { method: "PUT", body: JSON.stringify(payload) }); showToast("Customer updated"); }
    else { await apiFetch("/api/customers", { method: "POST", body: JSON.stringify(payload) }); showToast("Customer added"); }
